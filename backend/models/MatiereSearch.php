...
/**
 * MatiereSearch represents the model behind the search form of `backend\models\Matiere`.
 */
class MatiereSearch extends Matiere
{
    ...
    public function rules()
    {
        return [
            ...
            [['libelle', 'descriptions', 'key_matiere', 'created_at', 'updated_at'], 'safe'],
        ];
    }
    ...
    {
        if($additional == 1){
            $query = Matiere::find()->where(['statut' => 1]);
        }
        ...
        $query->andFilterWhere(['like', 'libelle',trim($this->libelle)])
            ...
            ->andFilterWhere(['like', 'key_matiere', $this->key_matiere]);
